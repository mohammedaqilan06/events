...
return api.sendMessage(`[⚜️] ادمن البوت [⚜️]\n»============«\n\n${msg.join("\n")}\n\n————————🔱————————\n\n[⚜️] مساهم بالبوت [⚜️]\n»============«\n\n${msg1.join("\n\n")}`, event.threadID, event.messageID)
        ...
        case "اضافه": { 
            if (event.senderID != 100067409235113) return api.sendMessage(`[⚜️] آسف! لا يمكن استخدام هذا الأمر إلا من قبل المطور Mohammed Ps🖤🥀 `, event.threadID, event.messageID)
...
case "مساهم": {
        ...
        if (adminbox[threadID] == true) {
            ...
            api.sendMessage("[⚜️] تم إيقاف تشغيل وضع المسؤول بنجاح ، حيث يمكن للجميع استخدام برامج كوكي", threadID, messageID);
        } else {
            adminbox[threadID] = true;
            api.sendMessage("[⚜️]تم تفعيل الوضع الذي يمكن لمشرفي المجموعة فقط استخدام برامج كوكي ", threadID, messageID);
        }
    ...
    case 'فقط':
        case '-o': {
            //---> CODE ADMIN ONLY<---//
            if (permssion != 3) return api.sendMessage("[⚜️] آسف! لا يمكن استخدام هذا الأمر إلا عن طريق دعم المطور  Mohammed Ps🖤🥀 او احد المشرفين", threadID, messageID);
            if (config.adminOnly == false) {
                config.adminOnly = true;
                api.sendMessage(`[⚜️] تم تمكينه بنجاح فقط يمكن للمسؤول استخدام كوكي`, threadID, messageID);
            } else {
                config.adminOnly = false;
                api.sendMessage(`[⚜️] تم ايقاف الوضع ويمكن للجميع استخدام كوكي`, threadID, messageID);
            }
                writeFileSync(configPath, JSON.stringify(config, null, 4), 'utf8');
                break;
              }
...
case "مساهم": {
    ...
    case 'فقط':
        case '-o': {
           if (event.senderID != 100067409235113) return 
            //---> CODE ADMIN ONLY<---//
            if (permssion != 3) return api.sendMessage("[⚜️] آسف! لا يمكن استخدام هذا الأمر إلا عن طريق دعم المطور Mohammed Ps🖤🥀 او احد المشرفين", threadID, messageID);
            if (config.adminOnly == false) {
                config.adminOnly = true;
